using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using LionvIDal.ProductionInfo;
using LionvFactoryDal;
using LionvModel.ProductionInfo;

namespace LionvBll.ProductionInfo
{
    public class Sys_RefSizeTransformBll
    {
        private readonly ISys_RefSizeTransformDal dal=DataProductionAccess.CreateSys_RefSizeTransform();
		#region  BasicMethod
		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(string rjcode)
		{
			return dal.Exists(rjcode);
		}

		/// <summary>
		/// 增加一条数据
		/// </summary>
		public int  Add( Sys_RefSizeTransform model)
		{
			return dal.Add(model);
		}

		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update( Sys_RefSizeTransform model)
		{
			return dal.Update(model);
		}
 
		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(string rjcode)
		{
			
			return dal.Delete(rjcode);
		}
 
		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public  Sys_RefSizeTransform Query(string where)
		{

            return dal.Query(where);
		}
 
		/// <summary>
		/// 获得数据列表
		/// </summary>
		public List< Sys_RefSizeTransform> QueryList(string strWhere)
		{
			return  dal.QueryList(strWhere);
		}
	 
		#endregion  BasicMethod
		#region  ExtensionMethod
        public int CreateCode()
        {
            return dal.CreateCode();
        }
        public bool SetMtMsSize(string mtcode, string mscode, string rjcode)
        {
            return dal.SetMtMsSize(mtcode,  mscode,  rjcode);
        }
        public bool ReSetMtMsSize(string mtcode, string mscode)
        {
            return dal.ReSetMtMsSize(mtcode, mscode);
        }
        public string GetMtMsSize(string mtcode, string mscode)
        {
            return dal.GetMtMsSize(mtcode, mscode);
        }
		#endregion  ExtensionMethod
    }
}
