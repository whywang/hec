...
{
    public class Sys_CgOverCondtionDal:ISys_CgOverCondtionDal
    {
        #region  BasicMethod
        /// <summary>
        /// 是否存在该记录
        ...
        public bool Exists(string where)
        {
            ...
            strSql.Append("select count(1) from Sys_CgOverCondtion");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Sys_CgOverCondtion(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Sys_CgOverCondtion set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Sys_CgOverCondtion ");
            strSql.AppendFormat(" where 1=1 {0} ", where);
            bool r = false;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,ccode,cname,oname,ocode,cattr,bvalue,tvalue,cfstr,cfscale,cxs,cpricetype,pricetx,pricetxtype,pcity,maker,cdate from Sys_CgOverCondition ");
            strSql.AppendFormat(" where 1=1 {0}", where);
            Sys_CgOverCondtion r = new Sys_CgOverCondtion();
        ...
        /// 得到一个对象实体
        /// </summary>
        public  Sys_CgOverCondtion DataRowToModel(DataRow row)
        {
             Sys_CgOverCondtion model = new  Sys_CgOverCondtion();
            if (row != null)
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Sys_CgOverCondtion> QueryList(string where)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,ccode,cname,oname,ocode,cattr,bvalue,tvalue,cfstr,cfscale,cxs,cpricetype,pricetx,pricetxtype,pcity,maker,cdate ");
            strSql.AppendFormat(" FROM Sys_CgOverCondtion where 1=1 {0}", where);
            List<Sys_CgOverCondtion> r = new List<Sys_CgOverCondtion>();
            DataTable dt = SqlHelp.GetDataTable(CommandType.Text, strSql.ToString(), null);
            if (dt != null)
            {
        ...
        public int CreateCode()
        {
            int r = -1;
            string sql = "";
            sql = "select isnull(max(convert(int,ccode)),0) as n from Sys_CgOverCondtion ";
        ...
        public int SetProductionOc(string icode, string pcode, string ocode)
        {
            ...
            sql.AppendFormat(" delete from Sys_RInventoryCgOverCondition where icode='{0}' and ocode='{1}';", icode, ocode);
            string[] clist = pcode.Split(';');
            for (int i = 0; i < clist.Length; i++)
            {
                sql.AppendFormat(" insert into Sys_RInventoryCgOverCondition (icode,pcode,ocode) values ('{0}','{1}','{2}') ;", icode, clist[i], ocode);
            }
            int r = 0;
            try
            {
        ...
        {
            string r = "";
            string sql = "select ocode from Sys_RInventoryCgOverCondition where pcode='" + pcode + "'";
        ...
        {
            string r = "";
            string sql = "select pcode from Sys_RInventoryCgOverCondition where ocode='" + ocode + "' and icode='" + icode + "'";
            DataTable dt = SqlHelp.GetDataTable(CommandType.Text, sql, null);
            if (dt != null)
            {
                foreach (DataRow dr in dt.Rows)
