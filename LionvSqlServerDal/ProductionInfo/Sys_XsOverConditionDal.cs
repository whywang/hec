...
{
    public class Sys_XsOverConditionDal : ISys_XsOverConditionDal
    {
        #region  BasicMethod
        /// <summary>
        /// 是否存在该记录
        ...
        public bool Exists(string where)
        {
            ...
            strSql.Append("select count(1) from Sys_XsOverCondition");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Sys_XsOverCondition(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Sys_XsOverCondition set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from Sys_XsOverCondition ");
        ...
        public Sys_XsOverCondition Query(string where)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,ccode,cname,oname,ocode,cattr,bvalue,tvalue,cfstr,cfscale,cxs,cpricetype,pricetx,pricetxtype,pcity,maker,cdate from Sys_XsOverCondition ");
            strSql.AppendFormat(" where 1=1 {0}", where);
            Sys_XsOverCondition r = new Sys_XsOverCondition();
        ...
        /// 得到一个对象实体
        /// </summary>
        public  Sys_XsOverCondition DataRowToModel(DataRow row)
        {
             Sys_XsOverCondition model = new  Sys_XsOverCondition();
            if (row != null)
            {
        ...
        public List<Sys_XsOverCondition> QueryList(string strWhere)
        {
            ...
            strSql.AppendFormat(" FROM Sys_XsOverCondition where 1=1 {0} ", strWhere);
            List<Sys_XsOverCondition> r = new List<Sys_XsOverCondition>();
            DataTable dt = SqlHelp.GetDataTable(CommandType.Text, strSql.ToString(), null);
            if (dt != null)
            {
        ...
        public int CreateCode()
        {
            int r = -1;
            string sql = "";
            sql = "select isnull(max(convert(int,ccode)),0) as n from Sys_XsOverCondition ";
        ...
        public int SetProductionOc(string icode, string pcode, string ocode)
        {
            ...
            sql.AppendFormat(" delete from Sys_RInventoryXsOverCondition where icode='{0}' and ocode='{1}';", icode, ocode);
            string[] clist = pcode.Split(';');
            for (int i = 0; i < clist.Length; i++)
            {
                sql.AppendFormat(" insert into Sys_RInventoryXsOverCondition (icode,pcode,ocode) values ('{0}','{1}','{2}') ;", icode, clist[i], ocode);
            }
            int r = 0;
            try
            {
        ...
        {
            string r = "";
            string sql = "select ocode from Sys_RInventoryXsOverCondition where pcode='" + pcode + "'";
        ...
        {
            string r = "";
            string sql = "select pcode from Sys_RInventoryXsOverCondition where ocode='" + ocode + "' and icode='" + icode + "'";
            DataTable dt = SqlHelp.GetDataTable(CommandType.Text, sql, null);
            if (dt != null)
            {
                foreach (DataRow dr in dt.Rows)
