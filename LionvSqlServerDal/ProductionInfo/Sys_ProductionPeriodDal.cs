...
{
    public class Sys_ProductionPeriodDal : ISys_ProductionPeriodDal
    {
        #region  BasicMethod
		/// <summary>
		/// 是否存在该记录
       ...
       public bool Exists(string where)
		{
			...
			strSql.Append("select count(1) from Sys_ProductionPeriod");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Sys_ProductionPeriod(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Sys_ProductionPeriod set ");
		...
		public bool Delete(string gqcode)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Sys_ProductionPeriod ");
		...
		/// 得到一个对象实体
		/// </summary>
		public  Sys_ProductionPeriod Query(string where)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,gqname,gqcode,fname,fcode,gqnum,maker,cdate from Sys_ProductionPeriod ");
            strSql.AppendFormat(" where 1=1 {0}", where);
            Sys_ProductionPeriod r = new Sys_ProductionPeriod();
		...
		/// 得到一个对象实体
		/// </summary>
		public  Sys_ProductionPeriod DataRowToModel(DataRow row)
		{
			 Sys_ProductionPeriod model=new  Sys_ProductionPeriod();
			if (row != null)
			{
        ...
        public List<Sys_ProductionPeriod> QueryList(string strWhere)
		{
            ...
            strSql.AppendFormat(" FROM Sys_ProductionPeriod where 1=1 {0}", strWhere);
            List<Sys_ProductionPeriod> r = new List<Sys_ProductionPeriod>();
            DataTable dt = SqlHelp.GetDataTable(CommandType.Text, strSql.ToString(), null);
            if (dt != null)
            {
        ...
        public int CreateCode()
        {
            int r = -1;
            string sql = "";
            sql = "select isnull(max(convert(int,gqcode)),0) as n from Sys_ProductionPeriod ";
        ...
        public bool SetPeriod(string icode, string fcode, string gqcode)
        {
            ...
            sql.AppendFormat(" delete from Sys_RProductionPeriod where fcode ='{0}' and icode='{1}' ;", fcode, icode);
            sql.AppendFormat(" insert into Sys_RProductionPeriod (icode,fcode,gqcode) values ('{0}','{1}','{2}') ;", icode, fcode, gqcode);
            bool r = false;
        ...
        public string GetPeriod(string icode, string fcode)
        {
            ...
            strSql.AppendFormat(" FROM Sys_RProductionPeriod where fcode='{0}' and icode='{1}'", fcode, icode);
            DataTable dt = SqlHelp.GetDataTable(CommandType.Text, strSql.ToString(), null);
            if (dt != null)
            {
                foreach (DataRow dr in dt.Rows)
