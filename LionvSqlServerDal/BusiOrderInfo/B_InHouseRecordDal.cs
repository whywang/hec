...
{
    public class B_InHouseRecordDal : IB_InHouseRecordDal
    {
        #region  BasicMethod
        /// <summary>
        /// 是否存在该记录
        ...
        public bool Exists(string where)
        {
            ...
            strSql.Append("select count(1) from B_InHouseRecord");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into B_InHouseRecord(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update B_InHouseRecord set ");
        ...
        public bool Delete(string where)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("delete from B_InHouseRecord where 1=1 {0}", where);
            bool r = false;
        ...
        public  B_InHouseRecord Query(string where)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,sid,ccode,bnum,rperson,rdate,remarke,maker,cdate from B_InHouseRecord ");
            strSql.AppendFormat(" where 1=1 {0}", where);
            B_InHouseRecord r = new B_InHouseRecord();
        ...
        /// 得到一个对象实体
        /// </summary>
        public B_InHouseRecord DataRowToModel(DataRow row)
        {
             B_InHouseRecord model = new  B_InHouseRecord();
            if (row != null)
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<B_InHouseRecord> QueryList(string where)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,sid,ccode,bnum,rperson,rdate,remarke,maker,cdate ");
            strSql.Append(" FROM B_InHouseRecord ");
            strSql.AppendFormat(" where 1=1 {0}", where);
            List<B_InHouseRecord> r = new List<B_InHouseRecord>();
            DataTable dt = SqlHelp.GetDataTable2(CommandType.Text, strSql.ToString(), null);
            if (dt != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select isnull(sum(bnum),0) as bnum FROM B_InHouseRecord ");
            if (strWhere.Trim() != "")
            {
